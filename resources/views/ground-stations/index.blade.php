...
<div class="stat-card rounded-2xl p-4 mb-4">
    <form method="GET" class="flex flex-col lg:flex-row gap-3">
        ...
        <a href="{{ route('ground-stations.index') }}" class="px-5 py-2.5 bg-slate-200 hover:bg-slate-300 dark:bg-white/10 dark:hover:bg-white/20 text-slate-700 dark:text-gray-300 rounded-xl text-sm font-medium transition-colors flex items-center justify-center">
            Reset
        </a>
    </form>
</div>
...
<div class="flex items-center justify-between mb-4">
    <p class="text-sm text-slate-600 dark:text-gray-400">Menampilkan <span class="text-slate-800 dark:text-white font-medium">{{ $groundStations->count() }}</span> dari <span class="text-slate-800 dark:text-white font-medium">{{ $groundStations->total() }}</span> ground station</p>
    <div class="flex gap-2">
        <div class="relative group">
            ...
            <div class="absolute right-0 top-full mt-2 w-40 bg-white dark:bg-space-800 border border-slate-200 dark:border-white/10 rounded-xl shadow-2xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                <a href="{{ route('ground-stations.export.excel') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 dark:hover:bg-white/10 text-sm text-slate-600 dark:text-gray-300 hover:text-slate-800 dark:hover:text-white transition-colors">
                    <i class="fas fa-file-excel text-green-500 dark:text-green-400"></i>Excel
                </a>
                <a href="{{ route('ground-stations.export.pdf') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 dark:hover:bg-white/10 text-sm text-slate-600 dark:text-gray-300 hover:text-slate-800 dark:hover:text-white transition-colors">
...
<div class="stat-card rounded-2xl overflow-hidden mb-6">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-transparent">
        <h3 class="text-sm font-semibold text-slate-800 dark:text-white flex items-center gap-2">
            <i class="fas fa-map-marked-alt text-blue-500 dark:text-blue-400"></i>
            Peta Lokasi Ground Station
        </h3>
    </div>
    <div id="map" style="height: 350px; z-index: 10;"></div>
</div>
        ...
        <table class="data-table w-full">
            <thead>
                <tr class="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10">
                    ...
                    <th class="text-left px-5 py-4 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Nama Stasiun</th>
                    <th class="text-left px-5 py-4 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Lokasi</th>
                    <th class="text-left px-5 py-4 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Negara</th>
                    <th class="text-left px-5 py-4 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Koordinat</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                ...
                <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-white/5">
                    <td class="px-5 py-4 text-sm text-slate-500 dark:text-gray-400">{{ $groundStations->firstItem() + $loop->index }}</td>
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-lg bg-purple-100 dark:bg-purple-500/20 flex items-center justify-center">
                                <i class="fas fa-broadcast-tower text-purple-600 dark:text-purple-400 text-sm"></i>
                            </div>
                            <p class="text-sm font-medium text-slate-800 dark:text-white">{{ $gs->name }}</p>
                        </div>
                    </td>
                    <td class="px-5 py-4 text-sm text-slate-600 dark:text-gray-300">{{ $gs->location }}</td>
                    <td class="px-5 py-4 text-sm text-slate-600 dark:text-gray-300">{{ $gs->country }}</td>
                    <td class="px-5 py-4">
                        <p class="text-xs font-mono text-slate-500 dark:text-gray-400">{{ number_format($gs->latitude, 4) }}°</p>
                        <p class="text-xs font-mono text-slate-500 dark:text-gray-400">{{ number_format($gs->longitude, 4) }}°</p>
                    </td>
                    <td class="px-5 py-4">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('ground-stations.show', $gs) }}"
                               class="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-100 text-blue-600 hover:bg-blue-200 dark:bg-blue-500/20 dark:text-blue-400 dark:hover:bg-blue-500/30 transition-colors">
                                <i class="fas fa-eye text-xs"></i>
                            </a>
                            <a href="{{ route('ground-stations.edit', $gs) }}"
                               class="w-8 h-8 flex items-center justify-center rounded-lg bg-yellow-100 text-yellow-600 hover:bg-yellow-200 dark:bg-yellow-500/20 dark:text-yellow-400 dark:hover:bg-yellow-500/30 transition-colors">
                                <i class="fas fa-edit text-xs"></i>
                            </a>
                            <form method="POST" action="{{ route('ground-stations.destroy', $gs) }}" onsubmit="return confirm('Hapus ground station ini?')">
                                @csrf @method('DELETE')
                                <button type="submit"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg bg-red-100 text-red-600 hover:bg-red-200 dark:bg-red-500/20 dark:text-red-400 dark:hover:bg-red-500/30 transition-colors">
    ...
    if (station.latitude && station.longitude) {
        ...
        marker.bindPopup(`
            <div style="font-family: Inter, sans-serif; min-width: 150px;">
                ...
                <span style="font-size: 11px; color: #64748b;">${station.location}, ${station.country}</span><br>
                <span style="font-size: 11px; color: #64748b;">${station.satellites_count || 0} satelit terhubung</span>
            </div>
        `);
    }
});
